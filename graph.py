...
class Graph:
    ...
    def _FordFulkerson(self, matrix, sink, source):
        max_flow = 0
        path = self.BFS(matrix, source, sink) #find a path from source to sink
        while path:
            path_flow = float("Inf")
            s = sink
            ...
            max_flow += path_flow
            v = sink
            while(v !=  source): #update capacity of the edges and reverse edges
                u = path[v]
    ...
    def valid_flow_transform(self, valid_matrix, og_matrix):
        ...
        og_valid = self.get_just_og_edges(valid_matrix) #get right orientation of edges

        for edge in self.original_edges:
            i, j = edge[0], edge[1]
            og_matrix[i][j][0] -= og_valid[i][j] #substract valid flow from capacity
            og_matrix[j][i][0] = og_valid[i][j] - og_matrix[i][j][1] #add the surplus to the reverse edge
    ...
    def get_just_og_edges(self, matrix):
        #cleans up the matrix to only contain the original edges and only their flow
    ...
    def reverse_flow(self, matrix):
        #reverses the flow of the matrix, FF returns the true flow in the wrong direction
        new_matrix = [[0 for i in range(len(self.vertex_names))] for j in range(len(self.vertex_names))]
        for edge in self.original_edges:
            i, j = edge[0], edge[1]
            new_matrix[i][j] = matrix[j][i][0]
    ...
    def max_flow(self):
        og_matrix = deepcopy(self.ad_matrix) #save a copy of the original matrix
